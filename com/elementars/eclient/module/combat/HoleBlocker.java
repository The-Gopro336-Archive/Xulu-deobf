...
public class HoleBlocker extends Module {
   ...
   public static boolean isExposed() {
      if (mc.world == null) {
         ...
         case NORTH:
            var1 = HoleBlocker.Offsets.NORTH;
            var2 = var1.length;

            for(var3 = 0; var3 < var2; ++var3) {
               var4 = var1[var3];
               if (mc.world.getBlockState(var0.add(var4.x, var4.y, var4.z)).getBlock() == Blocks.AIR) {
                  return true;
               }
            }
            ...
            return false;
         case SOUTH:
            var1 = HoleBlocker.Offsets.SOUTH;
            var2 = var1.length;
      ...
      if (mc.player != null && !ModuleManager.isModuleEnabled("Freecam")) {
         if ((Boolean)this.triggerable.getValue() && this.totalTicksRunning >= (Integer)this.timeoutTicks.getValue()) {
            ...
            for(var1 = 0; var1 < (Integer)this.blocksPerTick.getValue(); ++this.offsetStep) {
               ...
               boolean var3 = false;
               String var4 = mc.player.getHorizontalFacing().getName().toUpperCase();
               int var7;
               if (var4.equalsIgnoreCase("NORTH")) {
                  var2 = HoleBlocker.Offsets.NORTH;
                  var7 = HoleBlocker.Offsets.NORTH.length;
               } else if (var4.equalsIgnoreCase("EAST")) {
                  var2 = HoleBlocker.Offsets.EAST;
                  var7 = HoleBlocker.Offsets.EAST.length;
               } else if (var4.equalsIgnoreCase("SOUTH")) {
                  var2 = HoleBlocker.Offsets.SOUTH;
                  var7 = HoleBlocker.Offsets.SOUTH.length;
               } else if (var4.equalsIgnoreCase("WEST")) {
                  var2 = HoleBlocker.Offsets.WEST;
                  var7 = HoleBlocker.Offsets.WEST.length;
               } else {
                  var2 = HoleBlocker.Offsets.EAST;
                  var7 = HoleBlocker.Offsets.EAST.length;
               }

               if (this.offsetStep >= var7) {
                  this.offsetStep = 0;
                  break;
               }

               BlockPos var5 = new BlockPos(var2[this.offsetStep]);
               BlockPos var6 = (new BlockPos(mc.player.getPositionVector())).add(var5.x, var5.y, var5.z);
               if (this.placeBlock(var6)) {
                  ++var1;
               }
            }
   ...
   private static class Offsets {
      // $FF: synthetic field
      private static final Vec3d[] EAST = new Vec3d[]{new Vec3d(-1.0D, 1.0D, 0.0D), new Vec3d(-1.0D, 2.0D, 0.0D), new Vec3d(0.0D, 2.0D, 0.0D)};
      // $FF: synthetic field
      private static final Vec3d[] WEST = new Vec3d[]{new Vec3d(1.0D, 1.0D, 0.0D), new Vec3d(1.0D, 2.0D, 0.0D), new Vec3d(0.0D, 2.0D, 0.0D)};
      // $FF: synthetic field
      private static final Vec3d[] SOUTH = new Vec3d[]{new Vec3d(0.0D, 1.0D, -1.0D), new Vec3d(0.0D, 2.0D, -1.0D), new Vec3d(0.0D, 2.0D, 0.0D)};
      // $FF: synthetic field
      private static final Vec3d[] NORTH = new Vec3d[]{new Vec3d(0.0D, 1.0D, 1.0D), new Vec3d(0.0D, 2.0D, 1.0D), new Vec3d(0.0D, 2.0D, 0.0D)};
   }
}
