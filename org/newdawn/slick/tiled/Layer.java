...
public class Layer {
   ...
   public Layer(TiledMap var1, Element var2) throws SlickException {
      ...
      if (var17.equals("base64") && var18.equals("gzip")) {
         ...
         throw new SlickException(String.valueOf((new StringBuilder()).append("Unsupport tiled map type: ").append(var17).append(",").append(var18).append(" (only gzip base64 supported)")));
      }
   }
   ...
   private byte[] decodeBase64(char[] var1) {
      ...
      if (var7 != var4.length) {
         throw new RuntimeException(String.valueOf((new StringBuilder()).append("Data length appears to be wrong (wrote ").append(var7).append(" should be ").append(var4.length).append(")")));
      } else {
         return var4;
      }
   }
